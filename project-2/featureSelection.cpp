...
struct Node {
    unsigned classification; // actual class
    ...
    unsigned knn_classification; // class determined by k-Nearest Neighbor
};
...
std::vector<Node> get_nodes(std::string filename) {
    ...
    if (file.is_open()) {
        while(std::getline(file, line)) { // reads file line by line
            ...
            Node node;
            bool isClass = true; // intializes as true so first word of each line is considered a class
            while (ss >> data) {
                if (isClass) {
                    node.classification = std::stoi(data); // stores class as an integer
                    isClass = false; // sets to false so remaining words are interpreted as features
                }
                else { node.features.push_back(std::stod(data)); } // stores features as doubles
            }
